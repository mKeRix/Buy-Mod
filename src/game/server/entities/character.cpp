...
bool CHARACTER::spawn(PLAYER *player, vec2 pos, int team)
{
	...
	queued_weapon = -1;
	if(player->acc_data.upgrade[3] == 1 && !game.controller->is_cup)
	{
...
int CHARACTER::handle_ninja()
{
	...
	if ((server_tick() - ninja.activationtick) > (data->weapons.ninja.duration * server_tickspeed() / 1000) && (player->acc_data.upgrade[3] == 0 || game.controller->is_cup))
	{
		// time's up, return
	...
	if(player->acc_data.upgrade[7] == 1 && !game.controller->is_cup)
	ninja.currentmovetime;
	...
	if (ninja.currentmovetime > 0)
	{
		// Set player velocity
		if(player->acc_data.upgrade[7] == 1 && !game.controller->is_cup)
			core.vel = ninja.activationdir * 15.0f;
		else
		...
		// check if we hit anything along the way
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				int dmg = 0;
				dmg = data->weapons.ninja.base->damage;
				if(player->acc_data.upgrade[3] == 1 && !game.controller->is_cup)
					dmg = 6;
				if(player->acc_data.upgrade[7] == 1 && !game.controller->is_cup)
					dmg = 1;

				ents[i]->take_damage(vec2(0,10.0f), dmg, player->client_id,WEAPON_NINJA);
			}
		}
		return 0;
	}
...
void CHARACTER::do_weaponswitch()
{
	...
	if(weapons[WEAPON_NINJA].got && (player->acc_data.upgrade[3] == 0 || game.controller->is_cup)) // if we have ninja, weapon selection is only with upgrade 3 possible
		return;
...
void CHARACTER::fire_weapon()
{
	...
	bool fullauto = false;
	if(active_weapon == WEAPON_GRENADE || active_weapon == WEAPON_SHOTGUN || active_weapon == WEAPON_RIFLE || (active_weapon == WEAPON_NINJA && player->acc_data.upgrade[7] == 1 && !game.controller->is_cup) || (active_weapon == WEAPON_GUN && player->acc_data.upgrade[23] == 1 && !game.controller->is_cup))
		fullauto = true;
	...
	switch(active_weapon)
	{
		case WEAPON_HAMMER:
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				int dmg;
				dmg = data->weapons.hammer.base->damage;
				if(player->lvl < 20)
				{
					int i = 1+(rand()%(2-1+1));
					if(i == 1)
				...
				if(player->acc_data.upgrade[12] == 1)
					dmg = 5;
				
				if(game.controller->is_cup)
					dmg = 3;

				ents[i]->take_damage(vec2(0,-1.0f), dmg, player->client_id, active_weapon);
				vec2 dir;
			...
			//portal
			if(!game.controller->is_cup && player->acc_data.upgrade[27] == 1)
			{
				if(!player->port_1)
				{
					player->port_1_pos = core.pos;
					player->port_1 = true;
					player->port_tick = -1;
				}
				else if(player->port_1 && !player->port_2)
				{
					player->port_2_pos = core.pos;
					player->port_tick = 50*10+25;
				}
				else if(player->port_1 && player->port_2)
				{
					player->port_2 = false;
		...
		case WEAPON_GUN:
		{
			...
			if(player->acc_data.upgrade[13] == 1)
				dmg = 2;
		...
		{
			int shotspread ;
			if(player->acc_data.upgrade[11] == 1 && !game.controller->is_cup)
				shotspread = 3;
			else
				shotspread = 2;
			...
			for(int i = -shotspread; i <= shotspread; i++)
			{
				...
				int lifetime;
				if(player->acc_data.upgrade[5] && !game.controller->is_cup)
					lifetime = 3+50;
				else
					lifetime = (int)(server_tickspeed()*tuning.shotgun_lifetime);
		...
		case WEAPON_NINJA:
		{
			...
			if(player->acc_data.upgrade[7] == 0  || game.controller->is_cup)
	...
	if(weapons[active_weapon].ammo > 0 && (player->acc_data.upgrade[17] == 0 || game.controller->is_cup)) // -1 == unlimited
		weapons[active_weapon].ammo--;
	attack_tick = server_tick();
	if(!reload_timer && player->acc_data.upgrade[7] == 1 && active_weapon == WEAPON_NINJA && !game.controller->is_cup)
	{
		reload_timer = 0;
		return;
	}
	if(!reload_timer && player->acc_data.upgrade[23] == 1 && active_weapon == WEAPON_GUN && !game.controller->is_cup)
	{
		reload_timer = 3;
		return;
	}
...
void CHARACTER::tick()
{
	...
	if(player->balloon)
	{
		player->balloon_tick--;
		core.vel.y += -0.60;
		if(player->balloon_tick <= 0)
		{
			player->balloon = false;
			core.vel.y = 0;
		}
	}
	...
	if(player->freeze)
	{
		...
		if(!game.controller->is_cup)
		{
			if(player->freeze_tick == 150)
				game.create_damageind(pos, 0, 3);
			else if(player->freeze_tick == 100)
				game.create_damageind(pos, 0, 2);
			else if(player->freeze_tick == 50)
				game.create_damageind(pos, 0, 1);
			player->freeze_tick--;
			if(player->freeze_tick == 0)
			{
				player->freeze = false;
				core.vel.y -= 10;
				NETEVENT_EXPLOSION *ev = (NETEVENT_EXPLOSION *)game.events.create(NETEVENTTYPE_EXPLOSION, sizeof(NETEVENT_EXPLOSION));
				if(ev)
				{
					ev->x = (int)pos.x;
					ev->y = (int)pos.y;
	...
	if(game.controller->is_cup && game.controller->potcount > 0)
		player->freeze = true;
	else if(game.controller->is_cup && game.controller->potcount < 1)
		player->freeze = false;
	
	// emotion bot
	int emotion = 0+(rand()%(12-0+1));
	if(player->acc_data.upgrade[15] == 1 && server_tick()%(50*7) == 0)
	game.send_emoticon(player->client_id,emotion);

	// infinite jump
	if(core.jumped >= 2 && player->acc_data.upgrade[18] == 1 && !game.controller->is_cup)
		core.jumped = 1;

	// hook damage
	if(core.hooked_player != -1 && player->acc_data.upgrade[24] == 1 && server_tick()%(25) == 0 && !game.controller->is_cup)
		game.players[core.hooked_player]->get_character()->take_damage(vec2(0,0), 1, player->client_id, WEAPON_GAME);

	// ballon and freeze die tiles
	if(col_get_index(pos.x, pos.y) == TILE_BFD)
	{
		if(player->freeze || player->balloon)
		{
			game.create_sound(pos, SOUND_CTF_CAPTURE);
			if(player->balloon)
				game.players[player->balloon_id]->acc_data.money += 2;

			if(player->freeze)
				game.players[player->freeze_id]->acc_data.money += 2;

			game.send_chat_target(player->balloon_id, "You got 2 $!");
			player->acc_data.exp += 1;
			player->score += 1;
		}
		die(player->client_id, WEAPON_GAME);
	}

	// spider web event
	if(game.controller->web_tick > 0)
	{
		MAPITEM_LAYER_TILEMAP *tmap = layers_game_layer();
		
		core.hook_pos = vec2((tmap->width*64)/4,(tmap->height*64)/4);
		input.hook = 1;
	...
	if(!game.controller->is_cup)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
			if(game.players[i])
				if(game.players[i]->port_1 && game.players[i]->port_2)
				{
					if(distance(core.pos, game.players[i]->port_2_pos) < 60)
					{
						game.create_sound(pos, SOUND_PLAYER_DIE);
						core.pos = game.players[i]->port_1_pos;
		...
		if(player->port_1)
		{
			NETEVENT_EXPLOSION *ev = (NETEVENT_EXPLOSION *)game.events.create(NETEVENTTYPE_EXPLOSION, sizeof(NETEVENT_EXPLOSION));
			if(ev){
				ev->x = (int)player->port_1_pos.x;
				ev->y = (int)player->port_1_pos.y;}
		}
		...
		if(player->port_2)
		{
			NETEVENT_EXPLOSION *ev = (NETEVENT_EXPLOSION *)game.events.create(NETEVENTTYPE_EXPLOSION, sizeof(NETEVENT_EXPLOSION));
			if(ev){
				ev->x = (int)player->port_2_pos.x;
				ev->y = (int)player->port_2_pos.y;}
			game.create_damageind(player->port_2_pos, 1+(rand()%(30-1+1)), 1);
		}

		if(player->port_tick == 50*10 && player->port_1)
			player->port_2 = true;

		if(player->port_1 && player->port_tick > -1)
			player->port_tick--;

		if(player->port_1 && player->port_tick == 0)
		{
			player->port_tick = -1;
			player->port_2 = false;
			player->port_1 = false;
...
bool CHARACTER::increase_health(int amount)
{
	...
	if(player->acc_data.upgrade[player->HEALTH] == 0 || game.controller->is_cup)
	{
		if(health >= 10)
 			return false;
	...
	{
		if(health >= player->acc_data.upgrade[player->HEALTH]+10)
			return false;
		health = clamp(health+amount, 0, player->acc_data.upgrade[player->HEALTH]+10);
		return true;
	}
}
...
{
	if(player->acc_data.upgrade[player->ARMOR] == 0 || game.controller->is_cup)
	{
		if(armor >= 10)
 			return false;
	...
	{
		if(armor >= player->acc_data.upgrade[player->ARMOR]+10)
			return false;
		armor = clamp(armor+amount, 0, player->acc_data.upgrade[player->ARMOR]+10);
		return true;
	}
}
...
void CHARACTER::die(int killer, int weapon)
{
	...
	PLAYER *k = game.players[killer];
	int money = 0;
	float exp = 0;
	float wexp = 0;
	// TODO: write this smarter
	if(k->client_id != player->client_id)
	{
		...
		exp = 0.5/(game.players[killer]->lvl)*(player->lvl)+wexp/(game.players[killer]->lvl)*(player->lvl);
		...
		{
			k->acc_data.exp += exp;
			k->acc_data.money += money;
			k->set_lvl(k->acc_data.exp, false);
		}
	}
	player->balloon = false;
	player->freeze = false;
...
bool CHARACTER::take_damage(vec2 force, int dmg, int from, int weapon)
{
	...
	if(game.players[from]->acc_data.upgrade[26] == 1 && player->freeze && weapon == WEAPON_HAMMER && !game.controller->is_cup)
		player->freeze = false;
	...
	if(from == player->client_id && player->acc_data.upgrade[19] == 1 && !game.controller->is_cup)
		dmg = 0;

	if(game.controller->one_hit_tick > 0 && game.players[from]->client_id != player->client_id)
	{
		die(from, weapon);
		return false;
	}

	if(weapon == WEAPON_GUN && game.players[from]->acc_data.upgrade[8] == 1 && !game.controller->is_cup)
	{
		player->balloon = true;
		player->balloon_tick = 150;
		player->balloon_id = game.players[from]->client_id;
		if(active_weapon == WEAPON_NINJA)
		{
			weapons[WEAPON_NINJA].got = false;
			active_weapon = last_weapon;
	...
	if(weapon == WEAPON_RIFLE && game.players[from]->acc_data.upgrade[9] == 1 && !game.controller->is_cup)
	{
		player->freeze = true;
		player->freeze_tick = 200;
		player->freeze_id = game.players[from]->client_id;
		if(active_weapon == WEAPON_NINJA)
		{
			weapons[WEAPON_NINJA].got = false;
			active_weapon = last_weapon;
	...
	if(game.players[from]->lvl > player->lvl && abs(game.players[from]->lvl - player->lvl) >= 15 && dmg > 2 && !game.controller->is_cup)
		dmg = dmg/3*2;
	...
	if(health <= 0)
	{
		...
		if(player_state == PLAYERSTATE_CHATTING)
			chatkill = true;
		else
			chatkill = false;
		...
		if(chatkill == true && !game.controller->is_cup)
		{
			char buf[256];
			str_format(buf, sizeof(buf), "%s made a chatkill.", server_clientname(from));
			game.send_chat(-1, GAMECONTEXT::CHAT_ALL, buf);
			if(game.players[from]->acc_data.exp > 3)
				game.players[from]->acc_data.exp -= 3;
			else
				game.players[from]->acc_data.exp = 0;
		}
