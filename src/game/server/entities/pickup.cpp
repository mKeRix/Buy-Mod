...
void PICKUP::tick()
{
	...
	if(chr && chr->alive)
	{
		...
		case POWERUP_WEAPON:
			...
			{
				if(subtype == WEAPON_SHOTGUN && chr->player->acc_data.upgrade[0] == 0 && !game.controller->is_cup)
					break;
				if(subtype == WEAPON_GRENADE && chr->player->acc_data.upgrade[2] == 0 && !game.controller->is_cup)
					break;
				if(subtype == WEAPON_RIFLE && chr->player->acc_data.upgrade[1] == 0 && !game.controller->is_cup)
					break;
