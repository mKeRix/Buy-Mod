...
bool LASER::hit_character(vec2 from, vec2 to)
{
	...
	//swap
	if(owner_char->player->acc_data.upgrade[25] == 1 && !game.controller->is_cup)
	{
		vec2 tmp;
		tmp = owner_char->core.pos;
		owner_char->core.pos = hit->core.pos;
		hit->core.pos = tmp;
		owner_char->core.hook_state = HOOK_RETRACTED;
		owner_char->core.hook_pos = owner_char->core.pos;
	}
