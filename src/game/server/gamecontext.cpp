...
void GAMECONTEXT::create_dragon_explosion(vec2 p, int owner, int weapon, bool bnodamage)
{
	// create the event
...
void GAMECONTEXT::send_broadcast(const char *text, int cid, bool extra)
{
	NETMSG_SV_BROADCAST msg;
	msg.message = text;
	msg.pack(MSGFLAG_VITAL);
	server_send_msg(cid);
	if(!extra && controller->is_cup)
	{
		if(cid == -1)
		{
			for(int i = 0; i < MAX_CLIENTS; i++)
				if(game.players[i])
					game.players[i]->broadcast_count = 3*50;
		}
		else
			game.players[cid]->broadcast_count = 3*50;
	}
}
